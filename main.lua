...
local function ResetHouseStateTimer(house)
    ...
    SetTimeout(time, function()
        ...
        TriggerClientEvent('wtrp-houserobbery:client:ResetHouseState', -1, house)
    end)
end

-- Callbacks

QBCore.Functions.CreateCallback('wtrp-houserobbery:server:GetHouseConfig', function(_, cb)
    cb(Config.Houses)
end)

-- Events

RegisterNetEvent('wtrp-houserobbery:server:SetBusyState', function(cabin, house, bool)
    Config.Houses[house]["furniture"][cabin]["isBusy"] = bool
    TriggerClientEvent('wtrp-houserobbery:client:SetBusyState', -1, cabin, house, bool)
end)

RegisterNetEvent('wtrp-houserobbery:server:enterHouse', function(house)
    local src = source
    if not Config.Houses[house]["opened"] then
        ResetHouseStateTimer(house)
        TriggerClientEvent('wtrp-houserobbery:client:setHouseState', -1, house, true)
    end
    TriggerClientEvent('wtrp-houserobbery:client:enterHouse', src, house)
    Config.Houses[house]["opened"] = true
end)

RegisterNetEvent('wtrp-houserobbery:server:searchCabin', function(cabin, house)
    local src = source
    ...
    TriggerClientEvent('wtrp-houserobbery:client:setCabinState', -1, house, cabin, true)
end)

RegisterNetEvent('wtrp-houserobbery:server:removeAdvancedLockpick', function()
...
RegisterNetEvent('wtrp-houserobbery:server:removeLockpick', function(source)
    local Player = QBCore.Functions.GetPlayer(source)
...
QBCore.Functions.CreateUseableItem("mansionlaptop", function(source)
    ...
    local Player = QBCore.Functions.GetPlayer(src)
       ...
       TriggerClientEvent("wtrp-houserobbery:client:mansionhacking", src) 
